const featured = [
  { kicker: "ESSENTIALS", title: <>Change Your<br />World View</>, app: "Microsoft Word", sub: "Create & Recreate...", tone: "from-cyan-500 via-sky-500 to-blue-900", art: "word", icon: "microsoft-word.svg" },
  { kicker: "OUR FAVORITES", title: <>A New Way to Play<br />Classic Games</>, app: "Apple Arcade", sub: "Game Room · Play Classic Clas...", tone: "from-fuchsia-500 via-pink-500 to-purple-900", art: "arcade", icon: "apple-arcade.svg" },
  { kicker: "TRY NOW", title: <>Get Behind the<br />Wheel of an F1 Car</>, app: "JigSpace", sub: "Unforgettable 3D...", tone: "from-red-600 via-rose-700 to-zinc-950", art: "f1", icon: "jigspace.svg" },
  { kicker: "INCREDIBLE INDIE", title: <>Search for Clues<br />Blackbox</>, app: "Blackbox", sub: "Think outside the...", tone: "from-violet-600 via-fuchsia-700 to-slate-950", art: "blackbox", icon: "blackbox.svg" },
];

const favorites = [
  { name: "Rec Room", desc: "Play & Create Games with Friends", icon: "rec-room.svg" },
  { name: "Complete HeartX", desc: "Hands-on medical learning", icon: "complete-heartx.svg" },
  { name: "Simply Piano", desc: "Learn Songs F...", icon: "simply-piano.svg" },
];
...
export default function Home() {
  return <main className="scene-background relative flex min-h-screen items-center justify-center overflow-hidden px-5 py-16 text-white">
    <aside className="absolute left-[calc(50%-min(31vw,560px)-3.75rem)] top-[47%] z-10 hidden -translate-y-1/2 flex-col gap-1.5 rounded-full border border-white/20 bg-slate-700/50 p-1.5 shadow-lg md:flex">
    ...
    <section className="relative w-full max-w-[1120px] -translate-y-[3vh] rounded-[30px] border border-white/20 bg-[#9a9189]/[.92] px-5 pb-6 pt-4 shadow-2xl md:w-[62vw] md:px-6 md:pb-5">
      ...
      <div className="mt-3.5 grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-4">
        {featured.map((card) => <article key={card.app} className={`featured-card relative isolate overflow-hidden rounded-[19px] bg-gradient-to-br ${card.tone} p-3.5 shadow-lg`}>
          ...
          <div className="absolute bottom-0 left-0 right-0 z-10 flex h-[92px] items-center gap-2 border-t border-white/10 bg-[#17191d]/90 p-3"><img src={`/icons/${card.icon}`} alt={`${card.app} icon`} className="h-10 w-10 shrink-0 rounded-[9px] object-cover" /><div className="min-w-0 flex-1"><p className="truncate text-[11px] font-bold">{card.app}</p><p className="truncate text-[9px] text-white/75">{card.sub}</p></div><GetButton /></div>
        </article>)}
      </div>
      ...
      <div className="mt-2.5 flex flex-col gap-2.5 lg:flex-row">
        {favorites.map((item) => <article key={item.name} className="flex h-[62px] min-w-0 flex-1 items-center gap-2.5 rounded-[14px] bg-[#5b5149]/95 px-3 py-2 shadow-md"><img src={`/icons/${item.icon}`} alt={`${item.name} icon`} className="h-11 w-11 shrink-0 rounded-[10px] object-cover shadow-md" /><div className="min-w-0 flex-1"><h3 className="truncate text-[11px] font-bold">{item.name}</h3><p className="truncate text-[10px] text-white/75">{item.desc}</p></div><GetButton /></article>)}
      </div>
    </section>
